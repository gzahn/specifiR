...
#'
#' First the permutation test of the ISA is used to assign a specificity index to each taxon. These values are generated by subtracting 1 from the p-value from the ISA, with values closer to 1 indicating taxa that are more specific. The method is based on the ISA described by Dufrêne & Legendre (1997) and the permutation test is based on that described by McCune & Grace (2002). The taxon_specificity_index output provides a list of these taxa level indices.
#'
#' Community level specificity indices are then generated by using a CWM analysis with the taxon level specificity indices. Values closer to one indicate communities that are more host specific. This output can be found in the community_specificity_index output.
#'
