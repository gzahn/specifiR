...
#' taxon_specificity_index = Intermediate result (`comm_name` `iv.max` `p.value` `occurrence` `occurrence_groups` `taxon_index`), taxa that are more host specific will have a greater taxon_index;
...
#' @details
#' @details
#' SpecifiR is a function used to assign specificity values to both individual taxa and entire communities in community data sets. This method is derived from two separate analyses, the Indicator Species Analysis (ISA) and a Community Weighted Mean (CWM) analysis.
#'
#' First the permutation test of the ISA is used to assign a specificity value to each taxon. These values are generated by subtracting 1 from the p-value from the ISA, with values closer to 1 indicating taxa that are more specific. The method is based on the ISA described by Dufrêne & Legendre (1997) and the permutation test is based on that described by McCune & Grace (2002). The taxon_specificity_index result provides a list of these taxa level indices.
#'
#' Community level specificity values are then generated by using a CWM analysis with the taxon level specificity indices. Values closer to one indicate communities that are more host specific. This output can be found in the community_level_specificity index.
#'
