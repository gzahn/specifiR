...
#' Subsequent lines for long description.
#' This version of the function is a convenience wrapper for phyloseq users.
#'
#'
#' @import tidyr
#' @import magrittr
...
specifiR_physeq <-
  function(physeq,
    ...
    # extract grouping vector from phyloseq object
    groups <- sample_data(physeq) %>% as("data.frame") %>% pluck(groups)

    # check that otu_table is raw counts
